...
The deletion of derived state must not delete canonical financial records. A rebuild command should report what it reconstructed and any canonical records it could not parse.
...
2. validate required IDs, currencies, amounts, relationships, and invariants;
3. read the current canonical versions;
4. stage all affected file changes;
5. write a journal entry and commit marker;
6. atomically replace files where supported;
7. refresh derived state;
8. report the committed IDs and validation result.

### 11.2 Files to app

When a human or Hermes changes a supported Markdown field, the watcher or explicit refresh should detect the change, parse it, compare it with the last known baseline, validate it, and update the UI only after the canonical record is accepted. Malformed or ambiguous changes must remain visible in a review/error state and must not be silently normalized away.

### 11.3 Hermes to files to app

Hermes should normally call a documented local command or library interface. It should not write arbitrary SQL or bypass the sync core. A successful Hermes mutation must result in a canonical filesystem change, a deterministic validation result, and an app/index refresh. A rejected mutation must leave canonical financial data unchanged.
...
Match or propose entities and transactions
        ↓
Request review where ambiguous
        ↓
Create or update canonical records
        ↓
Link original evidence
        ↓
Update app and rebuildable indexes
```
...
OCR, parsing, and model extraction are proposals until deterministic validation and human review establish what may become canonical. Extraction results should be stored separately from the original so the original remains authoritative evidence.
...
The design must define field ownership and precedence rules, such as source-owned, imported-but-overridable, user-owned, or derived. A fresh import should update an imported field only under documented conditions and should preserve user-owned values.
...
4. **Validate** — check schema, references, permissions, accounting invariants, currency rules, conflicts, and evidence requirements.
5. **Preview** — return affected files, record diffs, balance effects, and warnings.
6. **Commit** — journal and atomically apply the approved change.
7. **Verify** — re-read canonical records and derived state, then return the committed result.
...
Define the evaluation rubric, threat model, accounting vocabulary, fixture policy, and project decision-record format. Investigate candidate upstream applications without selecting one prematurely.

### Phase 1 — Canonical format and invariants

Specify entity schemas, IDs, relationships, numeric precision, dates, provenance, schema versioning, core accounting invariants, and representative synthetic fixtures. Validate the design with accounting review.

### Phase 2 — File-native rules core

Implement parsing, validation, stable relationship resolution, typed read operations, deterministic calculations, and safe transactional writes. Confirm that derived state can be deleted and rebuilt.

### Phase 3 — Import and Raw workflow

Add source preservation, file hashing, duplicate detection, supported importers, extraction result storage, uncertainty states, review queue, evidence linking, and Raw folder watching.

### Phase 4 — Two-way synchronization

Implement app-to-file and file-to-app synchronization, content baselines, conflict handling, move and rename detection, journal recovery, and full mutation verification.
...
Expose constrained discovery, read, propose, validate, preview, commit, and verify commands. Add small-model fixtures and refusal behavior for ambiguous or unsupported operations.
